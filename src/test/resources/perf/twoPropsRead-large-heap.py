...
data = np.empty((8, 52), int)
data = np.loadtxt(open("twoPropsReading-small-heap.txt", "rb"), delimiter=";", dtype=float)

start = 2
end = 12

means = np.mean(data[:, start:end], 1)

stddevs = np.std(data[:, start:end], 1)
...
plt.ylabel('Time (ms)')
plt.title('Computing Degrees of 1000 Random Vertices, Two Rel Properties')
plt.legend(('Plain Database', 'Full Relcount'), loc=2)
plt.yscale('log')
plt.xscale('log')
...
simple = (1.0 / data[0:4, start:end]) / (1.0 / data[4:8, start:end])
