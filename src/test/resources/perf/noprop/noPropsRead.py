...
data[0, :, :] = np.loadtxt(open("noPropsReading-disk.txt", "rb"), delimiter=";", dtype=int, usecols=(range(1, 102)))
data[1, :, :] = np.loadtxt(open("noPropsReading-nocache.txt", "rb"), delimiter=";", dtype=int, usecols=(range(1, 102)))
data[2, :, :] = np.loadtxt(open("noPropsReading-strong-cache.txt", "rb"), delimiter=";", dtype=int,
                           usecols=(range(1, 102)))
...
plt.plot(xaxis, means[0, 13:26], c="green", ls=':', linewidth=2.0)
plt.plot(xaxis, means[1, 13:26], c="green", ls='-.', linewidth=2.0)
plt.plot(xaxis, means[2, 13:26], c="green", ls='-', linewidth=2.0)
plt.xlabel('Average Vertex Degree')
plt.ylabel('Time (microseconds)')
plt.title('Computing Degrees of 10 Random Vertices, No Rel Properties')
plt.legend(('Plain Database (No Cache)',
            'Plain Database (Low Level)',
            'Plain Database (High Level)',
            'Simple Relcount (No Cache)',
            'Simple Relcount (Low Level)',
            'Simple Relcount (High Level)'), loc=2)
plt.yscale('log')
plt.xscale('log')
