...
import com.graphaware.neo4j.relcount.full.internal.cache.ThresholdBasedRelationshipCountCompactor;
import com.graphaware.neo4j.relcount.full.internal.dto.relationship.*;
...
import static com.graphaware.neo4j.relcount.full.internal.dto.property.CompactiblePropertiesImpl.ANY_VALUE;
import static com.graphaware.neo4j.relcount.full.module.FullRelationshipCountModule.FULL_RELCOUNT_DEFAULT_ID;
import static org.junit.Assert.*;

/**
 * Counting-related tests for {@link com.graphaware.neo4j.relcount.full.internal.cache.FullRelationshipCountCache}.
 */
public class FullRelationshipCountCachingNodeTest {
    ...
    private RelationshipCountingNode<RelationshipDescription> countingNode() {
        return new FullCachedRelationshipCountingNode(database.getNodeById(0), DefaultFrameworkConfiguration.getInstance().createPrefix(FULL_RELCOUNT_DEFAULT_ID), DefaultFrameworkConfiguration.getInstance().separator());
    }

    private RelationshipCountCachingNode<CompactibleRelationship> cachingNode() {
        return new FullRelationshipCountCachingNode(database.getNodeById(0), DefaultFrameworkConfiguration.getInstance().createPrefix(FULL_RELCOUNT_DEFAULT_ID), DefaultFrameworkConfiguration.getInstance().separator(), new ThresholdBasedRelationshipCountCompactor(10));
    }
    ...
    public void shouldCorrectlyReportAggregatedCountsWhenGeneralityIsMixed() {
        ...
        assertEquals(20, countingNode().getRelationshipCount(wildcard("test2#OUTGOING#key3#" + ANY_VALUE))); //this includes undefined => potential idea for improvement, split ANY_VALUE into ANY_VALUE_INCLUDING_UNDEF and ANY_CONCRETE_VALUE
        assertEquals(20, countingNode().getRelationshipCount(literal("test2#OUTGOING#key3#" + ANY_VALUE)));
