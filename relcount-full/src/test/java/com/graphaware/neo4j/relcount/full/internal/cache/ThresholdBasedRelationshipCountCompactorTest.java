...
import com.graphaware.neo4j.relcount.full.internal.node.FullRelationshipCountCachingNode;
...
/**
 * Unit test for {@link com.graphaware.neo4j.relcount.full.internal.cache.ThresholdBasedRelationshipCountCompactor}.
 */
public class ThresholdBasedRelationshipCountCompactorTest {
    ...
    private RelationshipCountCachingNode<CompactibleRelationship> cachingNode(RelationshipCountCompactor compactor) {
        return new FullRelationshipCountCachingNode(database.getNodeById(0), DefaultFrameworkConfiguration.getInstance().createPrefix(FULL_RELCOUNT_DEFAULT_ID), DefaultFrameworkConfiguration.getInstance().separator(), compactor);
    }
    ...
    public void countShouldBeCompactedWhenThresholdIsReached() {
        ...
        assertTrue(cachingNode(compactor).getCachedCounts().containsKey(new CompactibleRelationshipImpl("test#OUTGOING#k1#" + ANY_VALUE, null, hash())));
